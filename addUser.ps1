...
foreach ($user in $data)
{
    ...
    elseif ($user -like "## ELEVES*") {
        ...
        if ($flag -eq "personnel") {
            $dn= "OU=Users,OU=Professeurs" + ",DC=" + $domain.split("=")[1].split("}")[0].split(".")[0] + ",DC=" + $domain.split("=")[1].split("}")[0].split(".")[1]
            ...
            New-ADUser -Name '$name' -GivenName '$givenName' -Surname '$surname' -SamAccountName '$samAccountName' -UserPrincipalName '$userPrincipalName' -AccountPassword (ConvertTo-SecureString -AsPlainText '$accountPassword' -Force) -Enable $true -ChangePasswordAtLogon $true -Path '$dn'
        }
